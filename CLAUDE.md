...
Usa el patrón: `oauth-start.php` → `oauth-callback.php` → `poll.php` (cron) → `list.php` (lectura por panel) → `mr.php` o equivalente (acción). (El sub-módulo `alibaba-rx` para pedidos de Alibaba se **anuló el 2026-06-18** — no resultó; se borró el código, la pestaña del panel y el cron del VPS.)
...
- **Debounce / coalescing** (`_enqueue_inbound` → `_debounced_process` → `_respond_to_buffered`): varias burbujas seguidas del mismo teléfono ("hola" / "busco sierra" / "circular") se juntan en **una sola** respuesta. Espera `WA_DEBOUNCE_SECONDS` (**default 7s desde 2026-06-16**, antes 3s) tras el último mensaje. **Invariante clave:** `_debounced_process` saca su task de `_debounce_tasks` **antes** del primer `await` de la respuesta, para que un mensaje nuevo nunca cancele una respuesta ya en curso (solo cancela el timer de debounce). El partner, escalado, fuera de horario y la llamada a Claude viven ahora en `_respond_to_buffered`, no en el webhook. **Por qué se subió a 7s (gotcha real, visto en prod):** con 3s, una **foto + texto** en burbujas separadas caía en **lotes distintos** porque la imagen entrante se baja de Meta con `await download_meta_media` **dentro del webhook** (`main.py` ~2570), ANTES de encolar, así que la burbuja de imagen entra tarde; el lote del texto ya había disparado → el bot contestaba **2-3 veces y se contradecía** ("no encontré" / "no recibí imagen" / dos listas de productos distintas / dos precios de envío). 7s absorbe la descarga + el ritmo de tecleo humano y da UNA respuesta coherente. Si reaparecen respuestas dobles, subir más la ventana o (mejor) encolar primero y bajar la imagen en background.
...
### wa-bot: disponibilidad de producto — `estado` (en_stock / por_encargo / no_disponible)

`search_products` y `_get_product_full` devuelven un campo **`estado`** (además del `disponible` booleano y el `stock` numérico que **nunca** se le dice al cliente):
- `en_stock` → `qty_available > 0`: hay físico, entrega rápida.
- `por_encargo` → vendible (`_es_disponible` true) pero `qty_available <= 0`: **el caso más común del catálogo**. El prompt obliga al bot a dar el precio **y aclarar** "lo trabajamos por encargo, no es entrega inmediata; el plazo lo confirma un compañero".
- `no_disponible` → `_es_disponible` false: no se puede vender ahora.

**Por qué (bug real 2026-06-16):** antes el prompt decía "si `disponible` es true, presentalo sin advertencias". Como casi todo el catálogo tiene `allow_out_of_stock_order=True` (→ `disponible` true aunque `qty=0`), el bot daba precios como si hubiera stock inmediato. Visto en prod: cliente preguntó por la prensa de banco A608, el bot dijo "Sí, está disponible", el cliente mostró que la web la marca "temporalmente no disponible" y el bot tuvo que disculparse. El estado `por_encargo` evita ese malentendido. (Tono también se suavizó a cálido/cercano en usted, sin emojis; temperatura 0.4 → 0.55.)

### wa-bot: búsqueda de productos — vocabulario cliente→catálogo (`search_products_odoo`)

`search_products_odoo` (`main.py`) tokeniza la consulta y hace **AND de cada token** contra `name`/`default_code` (estrategias 1-2), con fallbacks OR (3) y query cruda (4); re-rankea por `_score_producto`. Gotcha de fondo (**bug real 2026-06-16**): el cliente y el catálogo usan **palabras distintas**, y el AND exigía la palabra exacta del cliente. Ej: "cuchilla de **ruter** para hacer **paneles**" NO encontraba "**Fresas** para **panelar** puertas" (A431, etc.) porque (a) las brocas de router se llaman *fresa/broca*, **nunca "cuchilla"** (y "cuchilla" matchea otra categoría: hojas de cepilladora/sierra); (b) "router/ruter" **no va en el nombre de la broca** (`panelar` AND `router` = 0 productos); (c) "paneles" ≠ "panelar" en un ilike de substring. El bot entendía bien la intención pero escalaba a humano por no encontrarlo. Fix (2 capas en el tokenizer/search):
- **`_SYNONYMS`** (cliente→catálogo): `cuchilla/router/ruter → fresa/broca/trompo`, `prisionero → tornillo/esparrago`, `cazoleta → oculta/europea`. Solo en esa dirección.
- **`_stem()`** (raíz aproximada): recorta plurales/terminaciones verbales (`paneles`/`panelar`→`panel`, `puertas`→`puerta`, `fresas`→`fresa`); conservador, solo si el stem queda ≥4 chars.
- Ambas se aplican vía **`_expand_token()`** tanto en el filtro (`_and_por_token`, OR fallback) como en `_score_producto`. Más muletillas verbales en `_STOPWORDS` (`hacer`, `sirve`, `uso`…).
- Verificado contra Odoo real: la consulta textual del cliente ahora devuelve A386/A1037/A430/**A431** en el top por la Estrategia 1 (AND), sin romper "tarugos 3/8" ni otras. Para ampliar cobertura, agregar pares a `_SYNONYMS` (no hace falta tocar la lógica).

### wa-bot: badge de mensajes pendientes (pestaña + icono PWA) — 2026-06-16

El panel marca los no-leídos en **dos** lugares: el **título de la pestaña** (`(N) Panel · …`) y el **icono de la PWA** (Badging API). En `static/panel.js`, `updateBadges(s.unread)` (llamado en cada `loadStats()`) setea `document.title` + `navigator.setAppBadge(n)` / `clearAppBadge()`. Con la PWA **cerrada**, el badge lo pone el **service worker** (`SW_JS` en `main.py`, versión bumpeada a `wabot-v4`) en el handler `push`: lee `data.unread` y llama `self.navigator.setAppBadge(cnt)`. Para eso el webhook ahora incluye el conteo en el payload (`SELECT COUNT(*) FROM conversations WHERE unread>0` → `data.unread` de `send_push_notification`). Requisitos: el badge del **icono** necesita PWA instalada + notificaciones activas (funciona en Android Chrome y escritorio Chrome/Edge; iOS 16.4+); el `(N)` de la **pestaña** funciona en cualquier navegador. Tras desplegar hay que **recargar el panel una vez** para tomar el SW nuevo (`skipWaiting` + `clients.claim`). Cambió `main.py` (SW + payload) → necesitó rebuild, no hot-reload.
